...
async function collectLicenses(deps) {
   ...
   for (const name of deps) {
      ...
      results.push({ name, version: data.version || 'unknown', license: license || 'UNKNOWN', path: pkgPath, kind: 'npm' });
   }
   return results;
}
...
async function main() {
   ...
   let permissive = 0, unknown = 0, flagged = 0;
   for (const r of lic) {
   ...
   const urlsRx = /(github|gitlab|vercel|aws|azure|gcp|firebase|sentry|datadog|newrelic|segment|mixpanel|amplitude)\.[a-z]+/ig;
   let mdFiles = [];
   try {
   ...
   const patterns = [
      ...
      { name: 'CLA/cessão', rx: /(Contributor License Agreement|CLA|cess[aã]o de direitos|transfer[eê]ncia de direitos|assignment of rights)/i },
      { name: 'Proprietary-only', rx: /\b(nao autorizado|reservados todos os direitos|all rights reserved)\b/i }
   ];
   // arquivos de texto relevantes
   const { stdout: textList } = await execFileAsync('git', ['ls-files', '*.*']);
   const textFiles = textList.split(/\r?\n/).filter(Boolean).filter(f => /\.(ts|js|json|md|txt|cjs|mjs)$/i.test(f));
   const findings = [];
   for (const rel of textFiles) {
      let content = '';
      try { content = await fs.readFile(path.join(ROOT, rel), 'utf-8'); } catch { continue; }
