...
describe('detectorDependencias', () => {
  it('detecta import relativo longo', async () => {
    const traverseMock = vi.fn((node, visitors) => {
      visitors.ImportDeclaration({
        node: { source: { value: '../../../../foo' }, loc: { start: { line: 1 }, column: 1 } },
      });
    });
    ...
    const { detectorDependencias } = await import('../../src/analistas/detector-dependencias.js');
    const fakeAst = { node: { type: 'File' } };
    const ocorrencias = detectorDependencias.aplicar('', 'src/teste.js', fakeAst as any, '', {
      arquivos: [{ relPath: 'src/teste.js', ast: fakeAst }],
    } as any);
    expect(
      Array.isArray(ocorrencias) &&
        ocorrencias.some(
          (o: any) => o.tipo === 'aviso' && o.mensagem.includes('relativo sobe muitos'),
        ),
    ).toBe(true);
  });
  ...
  it('detecta import de arquivo inexistente', async () => {
    const traverseMock = vi.fn((node, visitors) => {
      visitors.ImportDeclaration({
        node: { source: { value: './naoexiste' }, loc: { start: { line: 1 }, column: 1 } },
      });
    });
    await vi.doMock('../nucleo/constelacao/traverse.js', () => ({ traverse: traverseMock }));
    await vi.doMock('node:path', () => {
    ...
    expect(
      Array.isArray(ocorrencias) &&
        ocorrencias.some((o: any) => o.tipo === 'erro' && o.mensagem.includes('inexistente')),
    ).toBe(true);
  });
  ...
  it('detecta import de .js em arquivo TS', async () => {
    const traverseMock = vi.fn((node, visitors) => {
      visitors.ImportDeclaration({
        node: { source: { value: './foo.js' }, loc: { start: { line: 1 }, column: 1 } },
      });
    });
    await vi.doMock('../nucleo/constelacao/traverse.js', () => ({ traverse: traverseMock }));
    await vi.doMock('node:path', () => {
  ...
  it('detecta require de arquivo inexistente', async () => {
    const traverseMock = vi.fn((node, visitors) => {
      visitors.CallExpression({
        node: {
          callee: { type: 'Identifier', name: 'require' },
          arguments: [{ type: 'StringLiteral', value: './naoexiste' }],
          loc: { start: { line: 1 }, column: 1 },
        },
      });
    });
  ...
  it('detecta require externo', async () => {
    const traverseMock = vi.fn((node, visitors) => {
      visitors.CallExpression({
        node: {
          callee: { type: 'Identifier', name: 'require' },
          arguments: [{ type: 'StringLiteral', value: 'lib-externa' }],
          loc: { start: { line: 1 }, column: 1 },
        },
      });
    });
    ...
    expect(
      Array.isArray(ocorrencias) &&
        ocorrencias.some(
          (o: any) => o.tipo === 'info' && o.mensagem.includes('Require de dependência externa'),
        ),
    ).toBe(true);
  });
  ...
  it('detecta import externo', async () => {
    const traverseMock = vi.fn((node, visitors) => {
      visitors.ImportDeclaration({
        node: { source: { value: 'lib-externa' }, loc: { start: { line: 1 }, column: 1 } },
      });
    });
    await vi.doMock('../nucleo/constelacao/traverse.js', () => ({ traverse: traverseMock }));
    await vi.doMock('node:path', () => {
    ...
    expect(
      Array.isArray(ocorrencias) &&
        ocorrencias.some(
          (o: any) => o.tipo === 'info' && o.mensagem.includes('dependência externa'),
        ),
    ).toBe(true);
  });
  afterEach(() => {
  ...
  it('detecta dependências de import e require', async () => {
    ...
    const { detectorDependencias, grafoDependencias } = await import(
      '../../src/analistas/detector-dependencias.js'
    );
    const fakeAst = { node: { type: 'File' } };
