...
describe('scanner — leitura de conteúdo fora de VITEST (branches)', () => {
  ...
  beforeAll(async () => {
    ...
    const { config } = await import('../../src/nucleo/constelacao/cosmos');
  ...
  it('lê conteúdo via lerArquivoTexto e captura erro no catch', async () => {
    ...
    const persist = await import('../../src/zeladores/util/persistencia');
    ...
    const { scanRepository } = await import('../../src/nucleo/scanner');
