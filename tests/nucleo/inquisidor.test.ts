...
import * as inquisidor from '../../src/nucleo/inquisidor.js';
...
vi.mock('../../src/nucleo/executor.js', () => ({
...
vi.mock('../../src/nucleo/constelacao/cosmos.js', () => ({
  config: {
    SCANNER_EXTENSOES_COM_AST: ['.ts', '.js'],
    GUARDIAN_BASELINE: 'baseline.json',
    ZELADOR_STATE_DIR: '.',
  },
}));
vi.mock('../../src/analistas/detector-estrutura.js', () => ({ detectorEstrutura: vi.fn() }));
vi.mock('../../src/analistas/detector-dependencias.js', () => ({ detectorDependencias: vi.fn() }));
...
describe('inquisidor', () => {
  ...
  it('iniciarInquisicao retorna resultado esperado', async () => {
    const resultado = await inquisidor.iniciarInquisicao('/fake', {
      includeContent: true,
  ...
  it('prepararComAst lida com erro em decifrarSintaxe e log.erro é chamado', async () => {
    ...
    vi.doMock('./parser.js', () => ({
      decifrarSintaxe: vi.fn(async () => {
        throw new Error('falha sintaxe');
      }),
    }));
    const { prepararComAst } = await import('../../src/nucleo/inquisidor.js');
  ...
  it('EXTENSOES_COM_AST usa padrão se config.SCANNER_EXTENSOES_COM_AST não for array', async () => {
    vi.doMock('./constelacao/cosmos.js', () => ({
      config: { SCANNER_EXTENSOES_COM_AST: undefined },
    }));
    const { prepararComAst } = await import('../../src/nucleo/inquisidor.js');
    const entries = [{ relPath: 'file1.ts', content: 'conteudo', fullPath: undefined }];
