...
import * as poda from '../../src/zeladores/poda.js';
...
describe('poda', () => {
  ...
  it('executarPodaCiclica executa fluxo real sem erro', async () => {
    ...
    const detectarFantasmas = vi.mocked(
      (await import('../../src/zeladores/fantasma.js')).detectarFantasmas,
    );
    detectarFantasmas.mockResolvedValueOnce({
      total: 1,
      fantasmas: [{ arquivo: 'b.txt', referenciado: false, diasInativo: 0 }],
    });
    const lerEstado = vi.mocked(
      (await import('../../src/zeladores/util/persistencia.js')).lerEstado,
    );
  ...
  it('executarPodaCiclica lida com nenhum arquivo para podar', async () => {
    ...
    const gerarRelatorioPodaMarkdown = vi.mocked(
      (await import('../../src/relatorios/relatorio-poda.js')).gerarRelatorioPodaMarkdown,
    );
    const gerarRelatorioPodaJson = vi.mocked(
      (await import('../../src/relatorios/relatorio-poda.js')).gerarRelatorioPodaJson,
    );
    await poda.executarPodaCiclica(true);
