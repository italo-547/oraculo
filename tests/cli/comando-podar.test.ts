// Removida cópia duplicada — este arquivo contém apenas a suite única de testes anteriormente definida above.
// SPDX-License-Identifier: MIT
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { Command } from 'commander';
import { comandoPodar } from '../../src/cli/comando-podar.js';
...
describe('comandoPodar', () => {
  ...
  it('log.sucesso é chamado quando não há arquivos órfãos', async () => {
    ...
    readlineAnswer = 's';
    const { removerArquivosOrfaos } = await import('../../src/zeladores/poda.js');
  ...
  it('log.info e log.aviso são chamados para arquivos órfãos', async () => {
    ...
    readlineAnswer = 's';
    const { removerArquivosOrfaos } = await import('../../src/zeladores/poda.js');
    vi.mocked(removerArquivosOrfaos).mockResolvedValueOnce({
      arquivosOrfaos: [{ arquivo: 'foo.txt', referenciado: false, diasInativo: 10 }],
    });
    const { log } = await import('../../src/nucleo/constelacao/log.js');
    const program = new Command();
    const aplicarFlagsGlobais = vi.fn();
