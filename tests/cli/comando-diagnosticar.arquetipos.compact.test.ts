...
describe('comandoDiagnosticar arquetipos modo compacto', () => {
  ...
  beforeEach(() => {
    ...
    vi.resetModules();
    logMock = { info: vi.fn(), sucesso: vi.fn(), aviso: vi.fn(), erro: vi.fn(), fase: vi.fn() };
    ...
    vi.mock('../../src/analistas/detector-estrutura.ts', () => ({ sinaisDetectados: [] }));
  ...
  it('lista arquétipos em linha compacta', async () => {
    ...
    // console.log('LOG.FASE:', logMock.fase.mock.calls.map(c => String(c[0])));
    const found = [...logMock.info.mock.calls, ...logMock.fase.mock.calls].some((c: any[]) => {
