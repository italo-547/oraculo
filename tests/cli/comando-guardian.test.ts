...
describe('comandoGuardian', () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock('../../src/nucleo/constelacao/log.js', () => ({
      log: {
        info: vi.fn(),
        sucesso: vi.fn(),
        aviso: vi.fn(),
    ...
    vi.doMock('../../src/nucleo/constelacao/cosmos.js', () => ({
      config: { GUARDIAN_BASELINE: 'baseline-test.json', ZELADOR_STATE_DIR: 'state-test-dir' },
    }));
    vi.doMock('../../src/nucleo/inquisidor.js', () => ({
  ...
  it('executa verificação padrão de integridade (status ok)', async () => {
    vi.doMock('../../src/guardian/sentinela.js', () => ({
      scanSystemIntegrity: vi.fn(async () => ({ status: 'ok' })),
      acceptNewBaseline: vi.fn(async () => undefined),
    }));
    const { comandoGuardian } = await import('../../src/cli/comando-guardian.js');
  ...
  it('aceita novo baseline quando --accept-baseline', async () => {
    ...
    expect(log.info.mock.calls.flat()).toEqual(
      expect.arrayContaining([expect.stringMatching(/aceitando.+baseline/i)]),
    );
    expect(log.sucesso.mock.calls.flat()).toEqual(
      expect.arrayContaining([expect.stringMatching(/novo baseline/i)]),
    );
  ...
  it('mostra diferenças quando --diff e há alterações', async () => {
    vi.doMock('../../src/guardian/sentinela.js', () => ({
      scanSystemIntegrity: vi.fn(async () => ({
        ...
        detalhes: ['arquivo1.ts', 'arquivo2.ts'],
      })),
      acceptNewBaseline: vi.fn(async () => undefined),
    }));
    const { comandoGuardian } = await import('../../src/cli/comando-guardian.js');
  ...
  it('status Aceito mostra mensagem de aceitação', async () => {
    ...
    await program.parseAsync(['node', 'cli', 'guardian']);
    expect(log.sucesso.mock.calls.flat()).toEqual(
      expect.arrayContaining([expect.stringMatching(/baseline.+aceito|atualizado/i)]),
    );
  });
