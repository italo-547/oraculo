...
describe('comandoDiagnosticar', () => {
  ...
  it('executa somente varredura com --scan-only (sem AST ou técnicas)', async () => {
    ...
    const { prepararComAst, executarInquisicao } = await import('../../src/nucleo/inquisidor.js');
  ...
  it('executa diagnóstico com guardian-check e cobre todos os status', async () => {
    ...
    const { scanSystemIntegrity } = await import('../../src/guardian/sentinela.js');
    const scanSystemIntegrityMock = vi.mocked(scanSystemIntegrity);
    const { IntegridadeStatus } = await import('../../src/tipos/tipos.js');
    // Ok
    const ts = new Date().toISOString();
    scanSystemIntegrityMock.mockResolvedValueOnce({ status: IntegridadeStatus.Ok, timestamp: ts });
    // Criado
    scanSystemIntegrityMock.mockResolvedValueOnce({
    ...
    scanSystemIntegrityMock.mockResolvedValueOnce({
      status: IntegridadeStatus.AlteracoesDetectadas,
      timestamp: ts,
    });
