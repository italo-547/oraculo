...
// Mocks hoisted
vi.mock('../../src/nucleo/constelacao/log.js', () => ({ log: H.log }));
vi.mock('chalk', () => {
  const bold = (x: any) => x;
...
vi.mock('../../src/guardian/sentinela.js', () => ({ scanSystemIntegrity: H.scanIntegrityImpl }));
vi.mock('../../src/relatorios/gerador-relatorio.js', () => ({
  gerarRelatorioMarkdown: H.gerarRelatorioImpl,
}));
vi.mock('../../src/zeladores/util/persistencia.js', () => ({ salvarEstado: H.salvarEstadoImpl }));
...
vi.mock('../../src/nucleo/constelacao/cosmos.js', () => ({ config: H.config }));
