...
describe('corrigirEstrutura - branches de erro e caminhos alternativos', () => {
    ...
    it('loga erro se mkdir falhar', async () => {
        ...
        const fileEntries = [{ relPath: 'src/e.ts', fullPath: '/tmp/src/e.ts', ast: undefined as any, content: '' }];
        await corrigirEstrutura(mapa, fileEntries, '/tmp');
        const { log } = await import('../nucleo/constelacao/log.js');
    ...
    it('loga erro se stat lançar erro inesperado (rename falha)', async () => {
        ...
        const fileEntries = [{ relPath: 'src/f.ts', fullPath: '/tmp/src/f.ts', ast: undefined as any, content: '' }];
        await corrigirEstrutura(mapa, fileEntries, '/tmp');
        const { log } = await import('../nucleo/constelacao/log.js');
    ...
    it('loga erro se destino já existir', async () => {
        ...
        const fileEntries = [{ relPath: 'src/g.ts', fullPath: '/tmp/src/g.ts', ast: undefined as any, content: '' }];
        await corrigirEstrutura(mapa, fileEntries, '/tmp');
        const { log } = await import('../nucleo/constelacao/log.js');
    ...
    it('loga erro se rename falhar', async () => {
        ...
        const fileEntries = [{ relPath: 'src/h.ts', fullPath: '/tmp/src/h.ts', ast: undefined as any, content: '' }];
        await corrigirEstrutura(mapa, fileEntries, '/tmp');
        const { log } = await import('../nucleo/constelacao/log.js');
        expect(log.erro).toHaveBeenCalledWith(expect.stringContaining('Falha ao mover'));
    });
    ...
    it('executa plugin como função direta (não default)', async () => {
        ...
        const fileEntries = [{ relPath: 'src/i.ts', fullPath: '/tmp/src/i.ts', ast: undefined as any, content: '' }];
        await corrigirEstrutura(mapa, fileEntries, '/tmp');
        // @ts-ignore
    ...
    it('executa plugin como objeto com default', async () => {
        ...
        const fileEntries = [{ relPath: 'src/z.ts', fullPath: '/tmp/src/z.ts', ast: undefined as any, content: '' }];
        await corrigirEstrutura(mapa, fileEntries, '/tmp');
        // @ts-ignore
    ...
    it('loga erro desconhecido do plugin', async () => {
        ...
        const fileEntries = [{ relPath: 'src/j.ts', fullPath: '/tmp/src/j.ts', ast: undefined as any, content: '' }];
        await corrigirEstrutura(mapa, fileEntries, '/tmp');
        const { log } = await import('../nucleo/constelacao/log.js');
        expect(log.aviso).toHaveBeenCalledWith(expect.stringContaining('⚠️ Plugin falhou: ./plugin-estranho.js'));
        config.STRUCTURE_PLUGINS = [];
        vi.unstubAllGlobals();
    });
});
