...
vi.mock('./fantasma.js', () => ({
    detectarFantasmas: vi.fn(async () => ({ fantasmas: [{ arquivo: 'a.txt', referenciado: false }] })),
}));
...
describe('poda', () => {
    ...
    it('executarPodaCiclica executa fluxo real sem erro', async () => {
        ...
        detectarFantasmas.mockResolvedValueOnce({ total: 1, fantasmas: [{ arquivo: 'b.txt', referenciado: false, diasInativo: 0 }] });
    ...
    it('executarPodaCiclica lida com nenhum arquivo para podar', async () => {
        ...
        lerEstado.mockResolvedValueOnce([]); // reativar
        lerEstado.mockResolvedValueOnce([]); // historico
        const gerarRelatorioPodaMarkdown = vi.mocked((await import('../relatorios/relatorio-poda.js')).gerarRelatorioPodaMarkdown);
        const gerarRelatorioPodaJson = vi.mocked((await import('../relatorios/relatorio-poda.js')).gerarRelatorioPodaJson);
        await poda.executarPodaCiclica(true);
    ...
    it('executarPodaCiclica lida com erro ao mover arquivo', async () => {
        ...
        const detectarFantasmas = vi.mocked((await import('./fantasma.js')).detectarFantasmas);
        detectarFantasmas.mockResolvedValueOnce({ total: 1, fantasmas: [{ arquivo: 'c.txt', referenciado: false, diasInativo: 0 }] });
        const lerEstado = vi.mocked((await import('./util/persistencia.js')).lerEstado);
        lerEstado.mockResolvedValueOnce([]); // anteriores
        lerEstado.mockResolvedValueOnce([]); // reativar
        lerEstado.mockResolvedValueOnce([]); // historico
        vi.mock('node:fs', () => ({
            promises: {
                mkdir: vi.fn(async () => undefined),
                rename: vi.fn(async () => { throw new Error('erro ao mover'); }),
            },
        }));
        const { log } = await import('../nucleo/constelacao/log.js');
        await poda.executarPodaCiclica(true);
