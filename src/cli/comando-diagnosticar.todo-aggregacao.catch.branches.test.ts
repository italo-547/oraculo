...
describe('comandoDiagnosticar agregação TODO_PENDENTE - catch silencioso', () => {
    ...
    it('se ocorrer erro na agregação, não quebra o fluxo', async () => {
        ...
        const ocorrencias = [
            { tipo: 'TODO_PENDENTE', mensagem: 'todo', relPath: 'f1.ts' },
        ];
        ...
        vi.doMock('../nucleo/constelacao/log.js', () => ({ log: { info: vi.fn(), sucesso: vi.fn(), aviso: vi.fn(), erro: vi.fn(), infoDestaque: vi.fn(), calcularLargura: vi.fn(), imprimirBloco: vi.fn() } }));
        vi.doMock('chalk', () => ({ default: { bold: (x: string) => x, cyan: { bold: (x: string) => x }, yellow: { bold: (x: string) => x }, green: { bold: (x: string) => x } } }));
