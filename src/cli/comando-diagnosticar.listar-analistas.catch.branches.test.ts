...
describe('comandoDiagnosticar – listar-analistas catch (branches)', () => {
  ...
  it('quando import de registry falha, com DEV_MODE=true, loga debug no catch', async () => {
    const logMock = { info: vi.fn(), sucesso: vi.fn(), aviso: vi.fn(), erro: vi.fn(), debug: vi.fn(), imprimirBloco: vi.fn() } as any;
    ...
    vi.doMock('../analistas/registry.js', () => { throw new Error('broken import'); });
