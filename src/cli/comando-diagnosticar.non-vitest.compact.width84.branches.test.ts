...
describe('comandoDiagnosticar molduras runtime com COMPACT_MODE usa largura 84 fallback', () => {
    ...
    it('usa 84 quando calcularLargura é ausente e COMPACT_MODE=true', async () => {
        ...
        vi.doMock('../nucleo/inquisidor.js', () => ({
            ...
            executarInquisicao: vi.fn(async () => ({ ocorrencias: [{ tipo: 'X', relPath: 'a.ts', mensagem: 'm' }], fileEntries: fakeEntries })),
        ...
        vi.doMock('../analistas/detector-arquetipos.js', () => ({
            detectarArquetipos: vi.fn(async () => ({
                melhores: [{ nome: 'cli-modular', confidence: 80, score: 40, missingRequired: [], matchedRequired: [], forbiddenPresent: [], anomalias: [] }],
                baseline: { version: 1, timestamp: new Date().toISOString(), arquetipo: 'cli-modular', confidence: 80, arquivosRaiz: ['README.md'] },
        ...
        vi.doMock('../nucleo/constelacao/log.js', () => ({
            log: {
                info: vi.fn(), sucesso: vi.fn(), aviso: vi.fn(), erro: vi.fn(), infoDestaque: vi.fn(),
