...
describe('ritualComando', () => {
    ...
    it('extractHandlerInfo cobre FunctionExpression e ArrowFunctionExpression', async () => {
        ...
        expect(extractHandlerInfo(funcExpr as any)).toEqual({ func: funcExpr, bodyBlock: funcExpr.body });
        expect(extractHandlerInfo(arrowFunc as any)).toEqual({ func: arrowFunc, bodyBlock: arrowFunc.body });
    });
    ...
    it('detecta comando válido com FunctionExpression', () => {
        vi.doMock('../nucleo/constelacao/traverse.js', () => ({
            traverse: (node: any, visitors: any) => {
                visitors.enter({ node: { type: 'CallExpression', callee: { type: 'Identifier', name: 'onCommand' }, arguments: [null, { type: 'FunctionExpression', body: { type: 'BlockStatement' } }] } });
            },
        }));
        const fakeAst = { node: { type: 'File' } };
        const ocorrencias = ritualComando.aplicar('', 'bot.js', fakeAst as any, '', undefined);
    ...
    it('detecta comando válido com ArrowFunctionExpression', () => {
        vi.doMock('../nucleo/constelacao/traverse.js', () => ({
            traverse: (node: any, visitors: any) => {
                visitors.enter({ node: { type: 'CallExpression', callee: { type: 'Identifier', name: 'registerCommand' }, arguments: [null, { type: 'ArrowFunctionExpression', body: { type: 'BlockStatement' } }] } });
            },
        }));
        const fakeAst = { node: { type: 'File' } };
        const ocorrencias = ritualComando.aplicar('', 'bot.js', fakeAst as any, '', undefined);
    ...
    it('ignora handler inválido (sem bloco)', () => {
        vi.doMock('../nucleo/constelacao/traverse.js', () => ({
            traverse: (node: any, visitors: any) => {
                visitors.enter({ node: { type: 'CallExpression', callee: { type: 'Identifier', name: 'onCommand' }, arguments: [null, { type: 'FunctionDeclaration', body: { type: 'NotBlock' } }] } });
            },
        }));
        const fakeAst = { node: { type: 'File' } };
        const ocorrencias = ritualComando.aplicar('', 'bot.js', fakeAst as any, '', undefined);
    ...
    it('ignora CallExpression cujo callee não é Identifier', () => {
        vi.doMock('../nucleo/constelacao/traverse.js', () => ({
            traverse: (node: any, visitors: any) => {
                visitors.enter({ node: { type: 'CallExpression', callee: { type: 'Literal', value: 42 }, arguments: [] } });
            },
        }));
        const fakeAst = { node: { type: 'File' } };
        const ocorrencias = ritualComando.aplicar('', 'bot.js', fakeAst as any, '', undefined);
    ...
    it('ignora comando com handler ausente', () => {
        vi.doMock('../nucleo/constelacao/traverse.js', () => ({
            traverse: (node: any, visitors: any) => {
                visitors.enter({ node: { type: 'CallExpression', callee: { type: 'Identifier', name: 'onCommand' }, arguments: [null] } });
            },
        }));
        const fakeAst = { node: { type: 'File' } };
        const ocorrencias = ritualComando.aplicar('', 'bot.js', fakeAst as any, '', undefined);
    ...
    it('ignora comando com handler que não é função', () => {
        vi.doMock('../nucleo/constelacao/traverse.js', () => ({
            traverse: (node: any, visitors: any) => {
                visitors.enter({ node: { type: 'CallExpression', callee: { type: 'Identifier', name: 'onCommand' }, arguments: [null, { type: 'Literal', value: 42 }] } });
            },
        }));
        const fakeAst = { node: { type: 'File' } };
        const ocorrencias = ritualComando.aplicar('', 'bot.js', fakeAst as any, '', undefined);
