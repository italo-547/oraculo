...
export function comandoMetricas() {
    return new Command('metricas')
        ...
        if (opts.json) {
            ...
            console.log(JSON.stringify({ total: lista.length, limite: opts.limite, historico: ultimos, agregados: agg }, null, 2));
            return;
        }
        log.info(`\n📊 Execuções registradas: ${lista.length}`);
        if (!lista.length) {
            log.aviso('Nenhum histórico de métricas encontrado ainda. Execute um diagnóstico com --metricas ativo.');
            return;
        }
        for (const h of ultimos) {
            const data = new Date(h.timestamp).toISOString();
            log.info(`- ${data} | arquivos=${h.totalArquivos} analise=${formatarDuracao(h.tempoAnaliseMs)} parsing=${formatarDuracao(h.tempoParsingMs)} cache(h/m)=${h.cacheAstHits}/${h.cacheAstMiss}`);
        }
        if (opts.analistas && agg) {
            log.info('\n🧠 Top analistas (por tempo acumulado):');
            for (const a of agg.topAnalistas) {
                log.info(`  • ${a.nome} total=${formatMs(a.totalMs)} média=${formatMs(a.mediaMs)} exec=${a.execucoes} ocorr=${a.ocorrencias}`);
            }
        }
        if (agg) {
