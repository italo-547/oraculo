    ...
    return new Command('podar')
        ...
        try {
            ...
            if (incluiNodeModules) {
                config.ZELADOR_IGNORE_PATTERNS = config.ZELADOR_IGNORE_PATTERNS.filter((p) => !/node_modules/.test(p));
                config.GUARDIAN_IGNORE_PATTERNS = config.GUARDIAN_IGNORE_PATTERNS.filter((p) => !/node_modules/.test(p));
            }
            ...
            if (resultadoPoda.arquivosOrfaos.length === 0) {
                ...
                if (config.REPORT_EXPORT_ENABLED) {
                    try {
                        ...
                        log.erro(`Falha ao exportar relatórios de poda: ${e.message}`);
                        throw e;
                    }
                }
                return;
            ...
            log.erro(`❌ Erro durante a poda: ${typeof error === 'object' && error && 'message' in error ? error.message : String(error)}`);
            if (config.DEV_MODE)
                console.error(error);
            process.exit(1);
        }
