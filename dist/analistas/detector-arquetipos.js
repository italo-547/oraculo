...
import { carregarArquetipoPersonalizado, obterArquetipoOficial, integrarArquetipos, } from '@analistas/arquetipos-personalizados.js';
...
function calcularConstantesAdaptativas(totalArquivos, totalDirs) {
    ...
    return {
        ...
        PENALIDADE_MISSING_REQUIRED_ADAPTADO: Math.max(PENALIDADE_MISSING_REQUIRED * (1 - tamanhoProjeto * 0.1), 5),
...
function scoreArquetipo(def, arquivos, sinaisAvancados) {
    ...
    if (sinaisAvancados) {
        ...
        const maturidadeProjeto = Math.min((sinaisAvancados.funcoes + sinaisAvancados.classes + sinaisAvancados.tipos.length) / 50, 3);
...
export async function detectarArquetipos(contexto, baseDir) {
    ...
    const sinaisAvancados = extrairSinaisAvancados(contexto.arquivos, await lerEstado(path.join(baseDir, 'package.json')), undefined, baseDir, arquivos);
    // Pontua todos os arquétipos disponíveis usando sinais avançados
    let candidatos = arquetiposParaAvaliar.map((def) => scoreArquetipo(def, arquivos, sinaisAvancados));
