...
export function comandoReestruturar(aplicarFlagsGlobais) {
    return new Command('reestruturar')
        ...
        try {
            ...
            analiseParaCorrecao.ocorrencias.forEach((occ) => log.info(`- [${occ.tipo}] ${occ.filePath ?? occ.arquivo ?? 'arquivo desconhecido'}: ${occ.mensagem}`));
            if (!opts.auto) {
            ...
            const resultadoCorrecao = await corrigirEstrutura(analiseParaCorrecao.ocorrencias);
