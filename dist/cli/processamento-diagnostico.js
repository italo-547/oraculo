    ...
    try {
            ...
            try {
                ...
                switch (resultado.status) {
                    ...
                    case IntegridadeStatus.Aceito:
                        log.aviso(`${log.simbolos?.aviso || '!'} Guardian: novo baseline aceito — execute novamente.`);
                        break;
                    case IntegridadeStatus.AlteracoesDetectadas:
                        log.aviso(`${log.simbolos?.erro || '❌'} Guardian: alterações suspeitas detectadas! Considere executar 'oraculo guardian --diff'.`);
                        totalOcorrencias++;
                        break;
                }
            }
            catch (err) {
                log.erro(`${log.simbolos?.erro || '❌'} Guardian bloqueou: alterações suspeitas ou erro fatal.`);
        ...
        if (config.SCAN_ONLY) {
            ...
            if (config.REPORT_EXPORT_ENABLED) {
                try {
                    ...
                    log.erro(`Falha ao exportar relatório de scan-only: ${e.message}`);
                }
            }
            if (opts.json) {
        ...
        if (arquetiposResultado) {
            ...
            if (config.VERBOSE && arquetiposResultado.candidatos?.length > 0) {
                ...
                if (candidatoTop.anomalias && candidatoTop.anomalias.length > 0) {
                    ...
                    if (candidatoTop.anomalias.length > 8) {
                        log.aviso(`Há ${candidatoTop.anomalias.length - 8} anomalias ocultas. Use --verbose para ver todas.`);
                    }
                }
            ...
            if (!opts.json &&
                ...
                if (arquetiposResultado.baseline) {
                    ...
                    linhasEstrutura.push(`Arquétipo baseline: ${baseline.arquetipo} (${baseline.confidence}%)`);
            ...
            if (opts.json) {
                ...
                try {
                    ...
                    if (metricasFinal && orig && Array.isArray(orig.analistas) && orig.analistas.length > 0) {
                        ...
                        metricasFinal = { ...metricasFinal, analistas: sorted };
                    }
        ...
        if (!opts.json && !config.SCAN_ONLY) {
            try {
                ...
                log.erro(`Falha ao exportar relatórios: ${e.message}`);
            }
        }
            ...
            try {
                if (!opts.json && !config.SCAN_ONLY) {
                    if (process.env.VITEST) {
                        ...
                        for (const p of candidates) {
                            try {
                                ...
                                if (l) {
                                    try {
                                        if (totalOcorrencias === 0)
                                            l.sucesso?.('Repositório impecável');
                                        else
                                            l.aviso?.(`Encontradas ${totalOcorrencias} ocorrências`);
                                    }
                                    catch { }
                                }
                            }
                        ...
                        try {
                            if (totalOcorrencias === 0)
                                logDyn?.sucesso?.('Repositório impecável');
                            else
                                logDyn?.aviso?.(`Encontradas ${totalOcorrencias} ocorrências`);
                        }
                        catch { }
                    }
                }
